...
program persuasio_yz, eclass
	...
	* inference based on bootstrap
	if "`method'" == "bootstrap" { 
		...
		if "`nboot'" != "" {
			bootstrap coef=e(lb_coef), reps(`nboot') level(`bs_level') notable nowarn: apr_lb `varlist'
		}
		if "`nboot'" == "" {
			bootstrap coef=e(lb_coef), reps(100) level(`bs_level') notable nowarn: apr_lb `varlist'
